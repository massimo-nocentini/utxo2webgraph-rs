//! `utxo2webgraph` binary.
//!
//! This file is dispatch and orchestration only: thread-pool setup, the
//! disk-space preflight, and the fused pipelines that `build_pg.sh` and
//! `builder.sh` used to drive as a chain of separate processes glued together
//! with GNU `sort`. Logger setup lives next door in [`logging`]; all of the
//! algorithm lives in the library.
//!
//! It is the only file in the crate that may use `anyhow`: the library returns
//! typed `Error`s so that `--lenient` and `--on-missing-source` can *match*
//! on them, and the binary adds human-readable context on top. Exit codes are
//! exactly 0 and 1 and nothing else, so a wrapper script can branch on success
//! without having to learn a status vocabulary.
//!
//! # Attribution
//!
//! The pipeline and the graph-construction algorithm this binary drives are
//! the work of **Matteo Loporchio**; what is here is orchestration around his
//! design.
...
use utxo2webgraph::{
    free_space_bytes, take_iter_error, total_memory_bytes, ArcSink, NodeId, ARC_RECORD_SIZE,
};
...
fn main() -> ExitCode {
    match run() {
        ...
        Err(e) => {
            ...
            // i.e. the line, the transaction, the input index and the missing
            // pair, rather than an anonymous failure somewhere in a 132 GB read.
            eprintln!("Error: {e:#}");
            ExitCode::from(1)
        }
    }
...
fn run() -> Result<()> {
    ...
    // 2. Logger. Never stdout: stdout carries the statistics lines and nothing
    //    else, so a log line there would break a naive diff against the
    //    historical pipeline logs. stderr always, plus a
...
/// Copies the statistics lines (which go to stdout) into the log, so
/// `<log-dir>/<stage>.log` is a complete record of the run on its own, without
/// the operator also having to have captured stdout.
fn log_stdout_stats(stats: &edge_list::EdgeListStats) {
    info!(
...
/// `arcs_per_run = memory / (2 * ARC_RECORD_SIZE)`.
///
...
///
/// An arc is [`ARC_RECORD_SIZE`] = 16 bytes, so a nominal 128 GiB budget
/// buffers `128 * 2^30 / 32` = 4.29e9 arcs per run. This is a plan number only:
/// `arcs::ArcSorter` computes the same quantity itself, and the two must agree
/// or the `--dry-run` plan and the preflight would describe a run that does not
/// happen.
fn arcs_per_run(memory: u64) -> u64 {
    (memory / (2 * ARC_RECORD_SIZE as u64)).max(1)
}
...
/// disk preflight and every `--dry-run` plan.
...
fn estimate(input_bytes: u64) -> Estimate {
    ...
    Estimate {
        ...
        arc_bytes: (arcs * ARC_RECORD_SIZE as f64) as u64,
    }
}
...
impl AtomicOut {
    ...
    fn new(final_path: &Path) -> Result<Self> {
        ...
        let work = final_path.with_file_name(format!(".{name}.u2wtmp{}", std::process::id()));
        Ok(Self {
            final_path: final_path.to_path_buf(),
            work_path: work,
            rename: true,
...
fn bvgraph_temp_basename(final_basename: &Path) -> PathBuf {
    ...
    final_basename.with_file_name(format!("{name}-u2wtmp{}", std::process::id()))
}
...
fn cmd_split(common: &CommonOpts, a: &SplitArgs) -> Result<()> {
    ...
    if common.dry_run {
        println!("DRY RUN: utxo2webgraph split");
        println!(
            "  input       : {} ({})",
            a.input.display(),
...
fn cmd_edge_list(common: &CommonOpts, a: &EdgeListArgs) -> Result<()> {
    ...
    if common.dry_run {
        println!("DRY RUN: utxo2webgraph edge-list");
    ...
    // left `..<name>.u2wtmp<pid>.utxo2webgraph-tmp-<pid>` orphans that nothing cleaned.
    create_parent(&a.arcs)?;
    let mut sink: Box<dyn ArcSink> = match a.arc_format {
        ArcFormatArg::Tsv => Box::new(TsvArcSink::create(&a.arcs)?),
        ArcFormatArg::Binary => Box::new(BinaryArcSink::create(&a.arcs)?),
    };
    ...
    // The clock starts just before the read loop and the node-map write is
    // INSIDE it, so the elapsed seconds on the statistics line cover the same
    // work as the historical pipeline logs an operator compares them against.
    let started = Instant::now();
    ...
    log_stdout_stats(&stats);
    report_edge_list_stats(&stats);
    Ok(())
}

/// Logs the counters that `--stats brief` cannot show without breaking the
/// byte-for-byte stdout contract its two lines are pinned to.
fn report_edge_list_stats(s: &edge_list::EdgeListStats) {
    ...
    if s.non_utf8_lines > 0 {
        warn!(
            "{} line(s) were not valid UTF-8 and were decoded with U+FFFD substitution; \
             run without --lenient to have the line and the byte offset within it reported \
    ...
    if s.dangling_refs > 0 {
        warn!(
            "{} dangling input reference(s) were tolerated; the default \
             --on-missing-source fail would have stopped at the first one",
            s.dangling_refs
        );
    }
    if s.zero_output_txs > 0 {
        warn!(
            "{} transaction(s) with no outputs emitted no edges, which is the rule: an empty \
             output section means no arcs, not one phantom edge into node 0 per input",
            s.zero_output_txs
        );
    }
    if s.distinct_nodes < s.node_slots {
        warn!(
            "`Nodes:` reports {} output slots, but only {} distinct nodes exist; the counter \
             is kept on slots for continuity with the historical pipeline logs, so a repeated \
             txId (the BIP-30 duplicates) overcounts it",
...
fn cmd_sort_edges(common: &CommonOpts, a: &SortEdgesArgs) -> Result<()> {
    ...
    if common.dry_run {
        println!("DRY RUN: utxo2webgraph sort-edges");
        println!(
            "  arcs        : {} ({})",
            a.input.display(),
            human_bytes(input_bytes)
        );
        println!("  sort algo   : {:?}", a.sort_algo);
        println!(
            "  memory      : {}  ({} arcs/run)",
            human_bytes(memory),
            arcs_per_run(memory)
    ...
    info!(
        "sorting with algo={:?} memory={} ({} arcs per run) threads={threads}",
        opts.algo,
        human_bytes(memory),
        arcs_per_run(memory)
    );
    ...
    let mut pl = progress_logger![display_memory = true, log_interval = common.log_interval];
    let (sorted, stats) = sort_arc_file(&a.input, a.input_format, opts, &mut pl)?;
...
fn sort_arc_file(
    ...
    match resolved {
        ...
        ArcFileFormat::Binary => {
            let (iter, slot) = utxo2webgraph::arcs::read_binary_arcs(path)?;
            for (src, dst) in iter {
                sorter.push(src, dst)?;
                pl.light_update();
            }
...
///
/// Sniffing only ever has to decide text versus binary: a binary arc file is
/// [`ARC_RECORD_SIZE`]-byte records and can be nothing else, so there is no
/// width left for the caller to get wrong and no flag that could disagree with
/// what is on disk.
fn resolve_format(path: &Path, format: InputFormatArg) -> Result<ArcFileFormat> {
    Ok(match format {
        InputFormatArg::Auto => {
            let f = utxo2webgraph::arcs::detect_format(path)?;
            debug!("sniffed {} as {f:?}", path.display());
            f
        }
        InputFormatArg::Tsv => ArcFileFormat::Tsv,
        InputFormatArg::Binary => ArcFileFormat::Binary,
    })
}
...
fn cmd_compress(common: &CommonOpts, a: &CompressArgs) -> Result<()> {
    ...
    if common.dry_run {
        println!("DRY RUN: utxo2webgraph compress");
        println!(
            "  edge list   : {} ({})",
            a.input.display(),
            human_bytes(input_bytes)
    ...
    let format = resolve_format(&a.input, a.input_format)?;
    ...
    if num_arcs == 0 && !a.allow_empty {
        // An empty arc stream reaches a text-edge-list reader as a parse
        // failure on line 1 ("expected an integer, found end of input"), two
        // stages downstream of the real problem and unrecognisable as "there
        // were no arcs". Say so here, where the answer is still obvious.
        return Err(utxo2webgraph::Error::EmptyEdgeList.into());
    }

    let from_arcs = if num_arcs == 0 { 0 } else { max_node_id + 1 };
    ...
    // Pass 2: stream the sorted arcs straight into the compressor. Nothing
    // materialises the graph in memory first, so peak RSS is the sort budget
    // and the compressor's own buffers, not a function of the arc count.
    ...
    let result = (|| -> Result<compress::CompressStats> {
        let stats = match format {
            ...
            ArcFileFormat::Binary => {
                let (iter, slot) = utxo2webgraph::arcs::read_binary_arcs(&a.input)?;
                let s = compress::compress_sorted_iter(iter, num_arcs, &tmp, &opts)?;
                take_iter_error(&slot)?;
                s
            }
...
fn scan_arcs(
    ...
    // `count` is a tally and stays `u64`; `max` is a node id and is therefore
    // a `NodeId`, so it compares against the endpoints with no cast in either
    // direction and has no width of its own to overflow.
    let mut count = 0u64;
    ...
    if format == ArcFileFormat::Binary {
        let len = fs::metadata(path)
            .with_context(|| format!("could not stat {}", path.display()))?
            .len();
        pl.expected_updates(Some((len / ARC_RECORD_SIZE as u64) as usize));
    }
...
/// Resolves `--num-nodes`.
///
/// `node-map` makes every UTXO a node; `from-arcs` is `max(endpoint) + 1`,
/// which is smaller because unspent outputs numbered above the highest arc
/// endpoint never appear in an arc. The choice changes the `.properties`
/// `nodes=` value and every node-indexed downstream array, so whichever arm
/// runs logs the number it picked.
///
/// Only the `node-map` and `Explicit` arms can log *both* candidates, because
/// only they have both numbers: `from-arcs` is computed from the arcs alone
/// and deliberately does not open the node map, which at `N = 28` is a 51 GiB
/// file that resolving a default should not have to read.
fn resolve_num_nodes(
    ...
    match spec {
        NumNodesSpec::FromArcs => {
            info!("num_nodes = {from_arcs} (from-arcs: max endpoint + 1)");
            Ok(from_arcs)
        }
...
fn node_map_next_id(path: &Path) -> Result<usize> {
    ...
    let reader = BufReader::with_capacity(READ_BUFFER, file);
    // The id column is a node id, so it is read as a `NodeId`; `rows` is a
    // tally of lines and stays `u64`.
    let mut max: NodeId = 0;
    let mut rows: u64 = 0;
    for (i, line) in reader.lines().enumerate() {
        ...
        let id: NodeId = id.parse().with_context(|| {
            format!(
...
fn cmd_build_pg(common: &CommonOpts, a: &BuildPgArgs) -> Result<()> {
    ...
    let est = estimate(input_bytes);
    check_max_map_count(threads, a.parallel);
    // Reported, not enforced. Node ids are `NodeId` (`usize`) end to end and
    // arcs are a 16-byte packing with no ceiling, so a large projection is a
    // size to plan disk and RAM around rather than a reason to refuse: at N=28
    // the node-map estimate is ~2.23e9 nodes, which is simply a big graph.
    debug!(
        "projected node count: ~{:.3e} output slots, with --num-nodes {:?}",
        est.outputs, a.num_nodes
    );
    ...
    if common.dry_run {
        println!("DRY RUN: utxo2webgraph build-pg");
        print_input_plan(&paths, input_bytes)?;
        println!(
            "  node map    : {}",
            if a.no_node_map {
    ...
    if arcs_per_run(memory) >= est.arcs as u64 {
    ...
    // Measured from before the read loop and including the node-map write, so
    // the fused pipeline's elapsed seconds mean the same thing as the
    // edge-list stage's do when run on its own.
    stats.elapsed_secs = started.elapsed().as_secs();
    edge_list::print_stats(&stats, common.stats_style());
    log_stdout_stats(&stats);
    report_edge_list_stats(&stats);
    ...
    let num_nodes = match a.num_nodes {
        ...
        NumNodesSpec::NodeMap => {
            announce_num_nodes(node_map_next_id, from_arcs);
            node_map_next_id
        }
...
///
/// # Why the name carries the record width
///
/// The binary arc format has no header and no magic number, so a reader can
/// only tell one record width from another by the file's length — and every
/// multiple of 16 is also a multiple of 8. An earlier version of this program
/// wrote 8-byte records (a node-id pair packed into a `u64`); this one writes
/// [`ARC_RECORD_SIZE`]-byte records. Handed a leftover 8-byte file under the
/// old name, the current reader would glue each pair of old arcs into one new
/// arc, halving the arc count and inventing source ids around `1.8e19`, then
/// either exhaust memory sizing the graph from them or write a silently wrong
/// one — and a file with an odd number of old arcs would fail as a torn write,
/// blaming the wrong thing.
///
/// Putting the width in the file name makes a stale artefact simply *not
/// found*, so a resume re-sorts from the edge list instead of misreading it.
/// Rename this if the record width ever changes again.
fn kept_arcs_path(tmp_dir: &Path, output_prefix: &Path) -> PathBuf {
    let stem = output_prefix
        .file_name()
        .map(|n| n.to_string_lossy().into_owned())
        .unwrap_or_else(|| "pg".to_string());
    tmp_dir.join(format!("{stem}_arcs{ARC_RECORD_SIZE}.bin"))
}
    ...
    match a.from_stage {
        ...
        Some(StageArg::SortEdges) => {
            ...
            let sort_args = SortEdgesArgs {
                input: arcs,
                // Binary: `--keep-intermediate` wrote it through
                // `SortedArcs::write_binary`, in the one record format the
                // readers here accept.
